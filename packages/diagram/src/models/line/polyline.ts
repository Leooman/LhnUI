...
import { Point, Line, Direction } from '../../class'
import { pointInLine } from '../../utils/utils'

const minDistance = 50

export function polyline(ctx: CanvasRenderingContext2D, l: Line) {
  ctx.strokeStyle = l.active ? config.lineActiveStroke : config.lineStroke
  ctx.beginPath()
...
export function polylineControlPoints(ctx: CanvasRenderingContext2D, l: Line) {
  ...
  for (const item of l.controlPoints) {
    ctx.beginPath()
    ctx.arc(item.x, item.y, config.anchorRadius, 0, Math.PI * 2)
    ctx.stroke()
    ctx.fill()
  }
  ctx.restore()
}
export function polylineEndPoints(ctx: CanvasRenderingContext2D, l: Line) {
  ctx.save()
  ctx.fillStyle = '#fff'
  ctx.strokeStyle = l.active ? config.lineActiveStroke : config.lineStroke
  ctx.lineWidth = 2
  _drawPoint(ctx, l.from)
  _drawPoint(ctx, l.to)
  ctx.restore()
}
function _drawPoint(ctx: CanvasRenderingContext2D, item: Point) {
  ctx.beginPath()
  ctx.arc(item.x, item.y, config.anchorRadius, 0, Math.PI * 2)
  ctx.stroke()
  ctx.fill()
...
function getNextPointByBottom(from: Point, to: Point) {
  ...
  if (from.y < to.y) {
    ...
    // The to point below the from point.
  } else if (to.direction === Direction.Up) {
    if (from.x < to.x) {
      return getHorizontalPoints(from, to)
    } else {
      const pts = getHorizontalPoints(to, from)
